...
class particle(evaluate_neuralnetwork):
    def __init__(self,  dim,  maxx, minx, netw, traindata, testdata, id_island, batch_size,learn_rate,rnn):
        ...
        np_pos = self.rnn.addnoiseandcopy(0.0 , 0.01)
        np_vel = self.rnn.addnoiseandcopy(0.0 , 0.01)
...
class distributed_neuroevo:
    ...
    def __init__(self,  pop_size, max_evals, traindata, testdata, learn_rate, batch_size ,netw, num_islands,meth,surrogate_topology,use_surrogate,compare_surrogate,save_surrogate_data,path):
        #FNN Chain variables
    ...
    def initialize_islands(self):
        ...
        if self.meth == 'PSO':
            for i in range(0, self.num_islands): 
                self.islands.append(neuroevolution(self.rnn,self.pop_size, self.num_param, self.island_numevals,  self.max_limits, self.min_limits, self.topology, self.traindata, self.testdata, self.batch_size, self.learn_rate ,self.parameter_queue[i],self.wait_island[i],self.event[i], i, self.swap_interval,self.surrogate_parameter_queues[i],self.surrogate_start[i],self.surrogate_resume[i],self.surrogate_interval,self.surrogate_prob,self.save_surrogate_data,self.use_surrogate,self.compare_surrogate,self.surrogate_topology,self.path))
        """        
        ## for the DE part
        if self.meth == 'DE':
            for i in range(0, self.num_islands): 
    ...
    def get_results(self):
        ...
        if self.compare_surrogate is True:
            ...
            """
            new_surg_compare = []
            for i in range(gen_value):
                for j in range(counter):
                    new_surg_compare.append(surg_compare[j,i,:])

            new_surg_compare = np.array(new_surg_compare)  
            surrogate_fit = new_surg_compare      
            """
            ...
            surrogate_plot = ax.plot(slen,surrogate_fit[:,1],linestyle='-', linewidth= 1, color= 'b', label= 'Surrogate ')
            model_plot = ax.plot(slen,surrogate_fit[:,0],linestyle= '--', linewidth = 1, color = 'k', label = 'True')

            #residuals =  surrogate_likl[:,0]- surrogate_likl[:,1]
            #res = ax.plot(slen, residuals,linestyle= '--', linewidth = 1, color = 'r', label = 'Residuals')
            ax.set_xlabel('Particles evolving over generations ',size= 25)
...
def main():
    ...
    if problem == "Time-Series":
        ...
        for run in range(1, 2) :  
            ...
            timer = time.time()
            neuroevolution =  distributed_neuroevo(pop_size, max_evals, traindata, testdata, learn_rate, batch_size ,netw, num_islands,method,surrogate_topology,use_surrogate,compare_surrogate,save_surrogate_data,path)
            ...
            allres =  np.asarray([ problem, run, train_per, test_per, train_per_std, test_per_std, timetotal,rmse_surr]) 
