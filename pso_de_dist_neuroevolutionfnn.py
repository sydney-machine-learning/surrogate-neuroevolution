...
class neuroevolution_de(evaluate_neuralnetwork, multiprocessing.Process):
    def __init__(self, pop_size, num_param, max_evals, max_lim, min_lim, netw, traindata, testdata, parameter_queue, wait_chain, event, island_id, swap_interval):
        ...
        #initializing variables for the de.
        self.pop_size = pop_size
        self.crossp = 0.7
        self.mut = 0.8
        self.dim = num_param
        self.bounds = [(min_lim[0],max_lim[0])]*num_param
    ...
    def run(self):  # this is executed without even calling - due to multi-processing
        ...
        while evals < self.max_evals:
            ...
            if evals % (self.pop_size * 2)  == 0: 
                ...
                print('evals_no:',evals,' ','epoch_no:', epoch,' ','island_id:',self.island_id,' ','train_perf:', float("{:.3f}".format(train_per)) ,' ','train_rmse:', float("{:.3f}".format(rmse_train)),' ' , 'classification_perf RMSE train * diff_e' ) 
...
class distributed_neuroevo:
    ...
    def __init__(self,  pop_size, dimen, max_evals,  max_limits, min_limits, netw, traindata, testdata, num_islands,meth):
        #FNN Chain variables
        self.traindata = traindata
        self.testdata = testdata
        self.topology = netw 
