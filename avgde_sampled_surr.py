...
class surrogate: #General Class for surrogate models for predicting likelihood(here the fitness) given the weights
    ...
    def create_model(self):
        ...
        #....................................#
        # The following topology if basically for training the sampled datasets with 4 moments as features.
...
class neuroevolution_de(evaluate_neuralnetwork, multiprocessing.Process):  # PSO http://www.scholarpedia.org/article/Particle_swarm_optimization
    ...
    def run(self): # this is executed without even calling - due to multi-processing
        ...
        trainset_empty = True
        is_true_fit = True
        surg_fit_list = [np.zeros((self.max_evals * 10,3)) for k in range(self.pop_size)]
        index_list = [0 for k in range(self.pop_size)]
        ...
        while evals < (self.max_evals ):
            ...
            for i in range(self.pop_size): # process each particle 
                ...
                #self.surrogate_prob = 0.5 + epoch/(2*(self.max_evals/self.pop_size))
                if ku<self.surrogate_prob and evals >= self.surrogate_interval+1:
                    ...
                    surr_proposal = self.fit_func(trial_denorm)
                    fitness_arr = np.array([surr_proposal])
                    if self.surrogate_topology is 4:
                        X, Y = trial_denorm,fitness_arr
                    else:
                ...
                if f < pop_de[i].fitness_value:
                    if is_true_fit == True:
                        ...
                        actual_fit = self.fit_func(trial_denorm)
                        if actual_fit < pop_de[i].fitness_value:
                            pop_de[i].fitness_value = actual_fit
                            pop_de[i].parameter = trial
                            if actual_fit < best_f:
                                best_f = f
                                best = copy.copy(pop_de[i].parameter)
